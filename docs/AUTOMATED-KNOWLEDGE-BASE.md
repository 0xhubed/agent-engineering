...
| Phase 4: Content Suggestions | ✅ Complete | Generate PRs for topic page updates |
...
### Phase 4: Content Suggestions (Week 7-8) - COMPLETED
...
**Files Created:**
- `scripts/content-agent.py` - Main content agent with confidence scoring and PR generation
- `.github/workflows/content-update.yml` - Runs after Update Resources or manual trigger
- `src/data/content-suggestions/` - Output directory for suggestion metadata

**Tasks:**
- [x] Create `scripts/content-agent.py`
- [x] Calculate confidence scores (bulk_score 40% + quality 40% + source_type 20%)
- [x] Generate content using Claude (prose + code examples in 3 languages)
- [x] Create PRs with copy-paste ready suggestions via gh CLI
- [x] Default to dry-run mode for safety

**Key Features:**
- **Confidence scoring:** Composite score from bulk analysis, deep dive quality, and source type
- **Framework standards:** Python uses LangChain, C# uses Microsoft Agent Framework
- **Safety first:** Defaults to dry-run mode, requires explicit `--create-pr` flag
- **Grouped PRs:** One PR per week with all suggestions organized by topic
...
# Actually create PR
python scripts/content-agent.py --create-pr

# Specific number of weeks
python scripts/content-agent.py --weeks 2 --create-pr
```

**Confidence Calculation:**
```python
confidence = (
    (bulk_score / 10) * 0.4 +    # From Tier 1 analysis (0-10)
    quality_score * 0.4 +         # Has code/techniques/applications (0-1)
    source_weight * 0.2           # paper=1.0, article=0.8, video=0.7, repo=0.6
)
```
...
### Topic: `memory`
**File:** `src/pages/topics/memory/index.astro`

#### Change 1: Hierarchical Memory Banks
**Source:** [Paper Title](https://arxiv.org/abs/...)
**Confidence:** 0.82
**Type:** paper

**Suggested prose:**
[2-3 paragraphs ready to copy]

**Suggested code example:**
<details>
  [Pseudo-code, Python, C# tabs]
</details>
...
- [ ] Ensure content fits page structure
```
...
│       └── content-update.yml      # Phase 4: Weekly PRs ✓
├── scripts/
...
│   │   │   └── YYYY-WW.json
│   │   └── content-suggestions/    # Phase 4: PR suggestion metadata ✓
│   │       └── YYYY-WW.json
│   └── pages/
│       └── resources/
